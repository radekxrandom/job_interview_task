...
const {synchronizeDatabaseTables, sequelizeConnection} = require('../utilities/database-connection');
...
test.serial('Retrieving all events in the db; GET /', async t => {
	...
	const {body: {events, count, limit, offset}, status} = res;
	t.is(status, 200);
...
test.serial('Checking validation when trying to create an event; POST /', async t => {
	let eventData = {	}
	const res = await request(app)
		.post('/')
		.send(eventData)
...
test.serial('Checking additional email validation when trying to create an event; POST /', async t => {
	const eventData = createFakeEvent();
